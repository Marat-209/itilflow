...
| Организационная единица | где живёт сам маршрут | от неё зависит, предложат ли маршрут в действии бизнес-правила |
| Виден в подчинённых единицах | **Да**, если правило будет в подразделении | иначе список маршрутов в действии правила окажется пустым |
...
![Карточка маршрута](img/ui-route-form.png)
...
| Статус объекта на этапе | **Согласование** |
...
**Статус объекта на этапе** переводит заявку в выбранный статус, как только
этап взят в работу: на согласовании — «Согласование», на ожидании ответа —
«Ожидание». Так по списку заявок видно стадию, не открывая каждую. Смена
пишется в ленту: «Статус: Новая → Согласование». Значение по умолчанию —
«Не менять»; завершающих статусов в списке нет, закрытие настраивается
у маршрута.
...
**Если согласующий заранее неизвестен**, поставьте в поле «Кто согласует»
значение «указывается при прохождении», а группу и сотрудника не заполняйте.
Этап откроется и будет ждать: на вкладке маршрута появится форма, где
исполнитель заявки или администратор процессов укажет согласующего
и обязательное основание выбора. Так делается согласование владельцем ресурса,
когда реестр владельцев ведётся вне GLPI.

В этой форме можно выбрать **сразу несколько сотрудников и групп** — каждому
уйдёт свой отдельный запрос. Пример: доступ к сетевой папке согласуют
руководитель заявителя и владелец папки, и это два независимых решения.
Сколько виз нужно для прохождения этапа, задаёт поле «Порог согласования»
этого же этапа: 100% — нужны все, 50% — достаточно половины. Пока виз
не хватает, в ленте заявки видно, сколько ещё ожидается.

**Отдельного права согласующему не нужно.** Достаточно того, что он назначен
согласующим на этапе. В GLPI 11 кнопки ответа показываются по проверке
`canAnswer()`, которая смотрит только на назначение и права профиля не учитывает.
Право «Согласовать запрос / инцидент» отвечает за другое — за то, кого GLPI
предлагает в своём выборе согласующего, а маршрут назначает согласующего сам
и этим выбором не пользуется. Проверено на 11.0.8: согласующий со штатным
профилем Technician отвечает нормально.
...
Так выглядит форма этапа целиком — на примере необязательного этапа закупки
в режиме «Дочерняя заявка»:

![Форма настройки этапа](img/ui-stage-form.png)
...
**Проверьте, что маршрут активен.** Неактивный маршрут правило назначит,
но `Engine::start()` его не запустит — и молча, без сообщения. Снаружи это
выглядит как «правило не работает». До версии 1.3.2 форма создавала маршрут
выключенным, поэтому на старых маршрутах это стоит перепроверить в первую
очередь: карточка маршрута, поле «Маршрут активен».

**Если список маршрутов в действии правила пуст** — маршрут лежит в другой
организационной единице и не помечен видимым в подчинённых. Выпадающий список
действия фильтруется по единице правила: правило в подразделении видит маршруты
своего подразделения и вышестоящих, но только те, у которых включена видимость
в подчинённых единицах. Это самая частая причина жалобы «правило не срабатывает»:
правило сохраняется без пригодного значения и молча ничего не делает.

У правил есть порядок и признак «остановить обработку». Если маршрут
не запускается при создании заявки — проверьте и это: правило могло перекрыть
другое, стоящее выше по порядку.
...
![Вкладка маршрута на заявке](img/ui-ticket-tab.png)
...
![Лист согласования](img/ui-approval-sheet.png)
...
| Согласующий не видит кнопок ответа | он не назначен согласующим на текущем этапе, либо этап уже закрыт | сверьтесь с вкладкой «Согласования» заявки: право профиля тут ни при чём |
...
| У дочерней заявки не тот SLA, что указан в этапе | SLA этапа выставляется при создании дочерней заявки, а затем его перекрывает бизнес-правило GLPI | посмотрите правила с действием «SLA», исключите из них категорию этапа или уберите SLA из этапа и положитесь на правила |
| Исполнитель или согласующий ничего не может сделать с заявкой | в организационной единице этой заявки у него только профиль самообслуживания | выдайте профиль технического специалиста в нужной единице: право зависит не только от профиля, но и от того, где он выдан |
